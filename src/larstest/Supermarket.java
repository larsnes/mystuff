...
public class Supermarket {
    ...
    private static class Product {
        ...
        final int     bulkRule;  // Pricing to apply for each qualifying bulk group,
                                 // e.g., bulkCount = 5 and bulkRule = 3 would
                                 // qualify
         ...
         */
        Product(String id, int unitPrice) {
            this.id = id;
         ...
         */
        Product(String id, int unitPrice, int bulkCount, int bulkRule) {
            this.id = id;
            this.unitPrice = unitPrice;
    ...
    private int computeProductPrice(Map.Entry<String, Integer> item)
        ...
        if (p != null) {
            if (p.hasBulk) {
                ...
                // the unit count qualifying for bulk pricing and bulkRule is 
                // the bulk pricing in unitPrice per bulk unit. bulkCount
            ...
            throw new Exception("Product " + item.getKey() + " not found!");
        }
    }
     ...
     * 
     * @param items - String of products, e.g., "ABBACBBAB"
    ...
    /**
     * Add a product to product "database", no special bulk pricing If a product with this id already exists in the
     * product "database", then this will replace the existing product
     * 
     * @param id - Product id, e.g., "A", must be single character
     * @param unitPrice - Regular unit price for this item
     * @throws Exception if product id is invalid
     */
    public void addProduct(String id, int unitPrice) throws Exception {

        if (id.trim().length() != 1) {
            throw new Exception("Invalid product id, must be single char: "
                    + id);
        }
    ...
    /**
     * Add a product to product "database", with special bulk pricing If a product with this id already exists in the
     * product "database", then this will replace the existing product
     * 
     * @param id - Product id, e.g., "A", must be single character
     * @param unitPrice - Regular unit price for this item
    ...
    public void addProduct(String id, int unitPrice, int bulkCount, int bulkRule)
        ...
        if (bulkCount <= 0) {
            throw new Exception(
                    "Invalid pricing rule, bulkCount must be greater than 0: "
                            + bulkCount);
        }
        if (bulkRule <= 0) {
            throw new Exception(
                    "Invalid pricing rule, bulkRule must be greater than 0: "
                            + bulkRule);
        }
